...
\[
 ...
 \tag{AI2}
\]
Here the first line is Mertens' theorem and its dyadic consequence, the first
estimate in (AI2) is the standard dyadic semiprime upper bound, and the last
is the prime number theorem in dyadic intervals.  No prime theorem in an
...
\[
  ...
  \tag{1}
\]
Regular variation, uniformly for $ak\leq i\leq bk$, also gives a constant
$\delta_{a,b}>0$ such that
\[
  \frac{\log Y}{\log Z}\geq1+\delta_{a,b}.
  \tag{2}
\]
Indeed the leading ratio is
$(k-i)/i\geq(1-b)/b>1$.
...
\begin{corollary}[Finite-certificate interface; conditional]
\label{cor:finite-certificate-conditional}
Assume the finite-block reduction in the stage manuscript, including its
$o(X_k)$ adjacent- and terminal-strip error estimates.  For every retained
middle-anchor triple $(i,k,d)$, suppose that one can choose a finite
\[
 F_{i,k,d}\subseteq\bigcup_hD_{i,h}(d)
\]
such that the corresponding actual maximal-anchor images satisfy
\[
 \lim_{J\to\infty}\sup_{k>J}\frac1{X_k}
 \left|\bigcup_{\substack{i\geq J\\(i,d)\ \mathrm{retained}}}
 d\bigl(I_{Y_{i,k,d}}\cap E(F_{i,k,d})\bigr)\right|=o(1)
\]
(equivalently, with the right-hand side written as $0$).  Then the
logarithmic-barrier profile is admissible.
\end{corollary}

\begin{proof}
An actual middle-anchor quotient avoids the full generator union and hence
avoids its subset $F_{i,k,d}$, so its terminal value lies in the displayed
actual union.  That union and the two assumed error strips are $o(X_k)$;
the conditional stage reduction then leaves a fixed positive proportion of
the terminal pool, which is precisely admissibility.
\end{proof}
...
the actual-union hypothesis of Corollary~\ref{cor:finite-certificate-conditional}.
Proposition~\ref{prop:semiprime-floor} rules out replacing that hypothesis by
