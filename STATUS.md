...
1. **Quasi-primitive embedding.** Prove the universal hereditary batch
   ...
   B15 reduces the one-root arbitrary-rank star problem to a directed fragile-surplus graph and solves every palette with one safe deletion prime per member. Whether every primitive pairwise non-coprime palette admits two injective primitive shadow classes remains open; a strict counterexample would refute universal HBC, though not the original domination conjecture.
   ...
   B18 raises the paper boundary to `|B|<=5`, uniformly over every intrinsic-height selector. At six points the first unhandled shapes include a five-child star, three same-word nodes split among three parents, and two consecutive non-singleton levels with unequal inherited deletion products. None is known to be an obstruction.
